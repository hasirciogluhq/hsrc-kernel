...
{
    return (int)strlen(s ? s : "") * 8 * scale;
}

void register_target(int y, int h, int setting_index)
{
    if (g_target_count >= (int)(sizeof(g_targets) / sizeof(g_targets[0])))
        return;
    g_targets[g_target_count].y = y;
    g_targets[g_target_count].h = h;
    g_targets[g_target_count].setting_index = setting_index;
    g_target_count++;
}
...
void draw_row(Surface &s, int y, const char *label, const char *value, bool interactive, bool hover)
{
    ...
    s.text(value_x, y + 14, value, interactive ? kAccent : kTextDim, 1);
}
...
int target_setting_hit(int lx, int ly)
{
...
void draw_sidebar(Surface &s)
{
    ...
    s.text(18, 18, "System Settings", kText, 2);
    s.text(18, 42, "Ring-3 control center", kTextDim, 1);
...
void draw_cycle_page(Surface &s, const char *subtitle)
{
    int y = kRowsStartY;
    s.text(kContentX, 48, kCategories[g_active_category].label, kText, 2);
    s.text(kContentX, 70, subtitle, kTextDim, 1);
    g_target_count = 0;
...
void draw_display_page(Surface &s)
{
    ...
    s.text(kContentX, 48, "Display", kText, 2);
...
{
    s.text(kContentX, 48, "Network", kText, 2);
...
{
    s.text(kContentX, 48, "Storage", kText, 2);
...
void draw_about_page(Surface &s)
{
    ...
    s.text(kContentX, 48, "About", kText, 2);
...
void paint()
{
    ...
    s.clear(kBg);
    draw_sidebar(s);
    s.fill(kSidebarW, 0, kWinW - kSidebarW, kHeaderH, kWhite);
    s.fill(kSidebarW, kHeaderH - 1, kWinW - kSidebarW, 1, kBorder);

    switch (g_active_category) {
    case CAT_GENERAL:
        draw_cycle_page(s, "Click any row to cycle and persist a value.");
        break;
    case CAT_KEYBOARD:
        draw_cycle_page(s, "Layout and repeat values are stored in /etc/os-settings.ini.");
        break;
    case CAT_MOUSE:
        draw_cycle_page(s, "Mouse preferences are persisted as simple key/value pairs.");
        break;
    case CAT_DISPLAY:
        draw_display_page(s);
        break;
    ...
    case CAT_DESKTOP_DOCK:
        draw_cycle_page(s, "Desktop shell can consume these persisted preferences later.");
        break;
    case CAT_STORAGE:
        draw_storage_page(s);
        break;
    case CAT_DATE_TIME:
        draw_cycle_page(s, "Clock and timezone choices are usermode preferences for now.");
        break;
    case CAT_SOUND:
        draw_cycle_page(s, "Audio controls are placeholders until deeper device APIs arrive.");
        break;
    case CAT_ABOUT:
        draw_about_page(s);
        break;
