...
#define ROT_ANG_PER_SEC                                                        \
  61440 /* ~0.94 rev/s - matches prior visual spin rate                        \
         */
...
static void put_px(uint32_t *fb, uint32_t stride, uint32_t w, uint32_t h, int x,
                   int y, uint32_t c) {
...
static void blend_px(uint32_t *fb, uint32_t stride, uint32_t w, uint32_t h,
  ...
  dg = (int)((dst >> 8) & 0xFFu);
  db = (int)(dst & 0xFFu);
  out = 0xFF000000u | (uint32_t)((dr + (sr - dr) * alpha_q8 / AA_Q8) << 16) |
...
static int sample_fill_cov_q8(int x, int y, int cx, int cy, int head_ang,
                              int arc_ang) {
  int dx = x - cx;
  int dy = y - cy;
  int dist;
...
void boot_splash_show(void) {
  ...
  for (;;) {
    ...
    draw_fill_arc(fb, mode.width, mode.width, mode.height, cx, cy, pct,
                  rot_ang);
