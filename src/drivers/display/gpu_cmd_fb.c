...
typedef struct {
    ...
    int has_rt, has_vb, has_tex, has_scissor;
...
static void put_px(gpu_tex_view_t *rt, int x, int y, uint32_t c, uint32_t blend)
{
    uint32_t *row;
    if (!rt || !rt->data || x < 0 || y < 0 ||
        (uint32_t)x >= rt->width || (uint32_t)y >= rt->height)
        return;
    row = (uint32_t *)((uint8_t *)rt->data + (uint32_t)y * rt->stride);
    row[x] = (blend == 0) ? (c | 0xff000000u) : blend_alpha(c, row[x]);
}

static uint32_t sample_tex(const gpu_tex_view_t *tex, int32_t u_q16, int32_t v_q16,
                           uint32_t vert_color)
{
    int32_t ui, vi;
    const uint8_t *p;
...
static void load_vtx(const uint8_t *base, uint32_t first, uint32_t i, vtx_t *o)
{
...
static int64_t edge(int32_t ax, int32_t ay, int32_t bx, int32_t by, int32_t cx, int32_t cy)
{
...
static void draw_tri(st_t *st, const vtx_t *a, const vtx_t *b, const vtx_t *c)
{
    ...
    for (y = miny; y <= maxy; y++) {
        for (x = minx; x <= maxx; x++) {
            ...
            if (textured) {
                int32_t uq = (int32_t)((w0 * a->u_q16 + w1 * b->u_q16 + w2 * c->u_q16) / area);
                int32_t vq = (int32_t)((w0 * a->v_q16 + w1 * b->v_q16 + w2 * c->v_q16) / area);
...
static int do_draw(st_t *st, uint32_t count, uint32_t first)
{
    ...
    if (!st->has_rt || !st->has_vb || !st->vb.data)
        return -1;
    if (st->shade >= 2 || st->topology != 0)
        return -1; /* 3D / non-triangle → unsupported here */
...
static int do_blit(const gpu_cmd_blit_t *cmd)
{
    ...
    for (y = 0; y < sh; y++) {
        ...
        uint32_t *drow =
            (uint32_t *)((uint8_t *)cmd->dst.data + (uint32_t)(dy + y) * cmd->dst.stride);
        for (x = 0; x < sw; x++) {
            uint32_t s = srow[sx + x];
            drow[dx + x] = (cmd->blend == 0) ? (s | 0xff000000u)
                                             : blend_alpha(s, drow[dx + x]);
        }
    }
    return 0;
}
...
int gpu_cmd_fb_exec(const void *gpu_cmds, uint32_t size)
{
    ...
    while (p + sizeof(gpu_cmd_hdr_t) <= end) {
        ...
        case GPU_CMD_CLEAR: {
            ...
            if (psz < sizeof(*c) || do_clear(&st, c->color_rgba) < 0)
                return -1;
            break;
        }
        case GPU_CMD_DRAW: {
            const gpu_cmd_draw_t *c = (const gpu_cmd_draw_t *)p;
            if (psz < sizeof(*c) || do_draw(&st, c->count, c->first) < 0)
                return -1;
            break;
        }
